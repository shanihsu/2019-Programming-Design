...
horse::horse(QIcon i, MainWindow *w, QGraphicsView *v, struct position p, bool color):chess(i, w, v, p, color)
{
    connect(b, SIGNAL(clicked()), this, SLOT(click_chess()));  //連接點擊滑鼠與棋子移動的訊號
    MainWindow::clickColor = chessData.color;  //記下滑鼠點到的棋子顏色
}

bool horse::judgeHorseRule(){
    if(MainWindow::clickColor == chessData.color)  //上一個棋子顏色跟現在相同,不可動
        return false;
    if(kingSeeKing() == true)  //如果王見王
        return false;
    if(chessData.color == true){  //棋子是黑色
        if((ix>-1 && ix<10) && (jy>-1 &&jy<9)){  //判斷滑鼠是否在可移動範圍內
            if((abs(ix-chessData.x1)==1 && abs(jy-chessData.y1)==2) || (abs(ix-chessData.x1)==2 && abs(jy-chessData.y1)==1)){  //走日字
                if(horseFeet() == false)  //沒拐馬腳
...
bool horse::horseFeet(){  //true:有拐馬腳,false:沒有拐馬腳
    if(MainWindow::pos[chessData.x1][chessData.y1-1].ChessExist == true && abs(ix-chessData.x1)==1 && chessData.y1-jy==2)  //左
        return true;
    else if(MainWindow::pos[chessData.x1][chessData.y1+1].ChessExist == true && abs(ix-chessData.x1)==1 && jy-chessData.y1==2)  //右
        return true;
    else if(MainWindow::pos[chessData.x1-1][chessData.y1].ChessExist == true && chessData.x1-ix==2 && abs(jy-chessData.y1)==1)  //上
        return true;
    else if(MainWindow::pos[chessData.x1+1][chessData.y1].ChessExist == true && ix-chessData.x1==2 && abs(jy-chessData.y1)==1)  //下
        return true;
    else {
        return false;
    }
...
bool horse::kingSeeKing()  //true:發生王見王, false::沒發生王見王
{
...
void horse::click_chess()
{
    ...
    if(moving == false)
        moving = true;
    else if(moving == true && judgeHorseRule() == true){  //如果符合規則
        moving = false;
